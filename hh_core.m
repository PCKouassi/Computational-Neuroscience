...
global Iinj0 Iinj1 Iinj2 Iinj3 Iinj4 time1 time2 time3 time4 duration1 duration2 duration3  duration4 V_insert tauH tauM Minfinity Hinfinity
...
total_current = Gl.*(V-El)+Gk.*n_gate.^4.*(V-Ek)+Gna*m_gate.^3.*h_gate.*(V-Ena)+Gca.*M_gate.^2.*H_gate.*(V-Eca);
...
% 
% subplot(3,1,3)
% plot(time,current_Na,'m', time,current_K,'r',time,current_leak,'b',time,current_Ca,'r',time, total_current, 'k');
% legend('INa','IK','IL','ICa','Total Current');
% axis([0 t_max 0 1]);
...
time=NaN(1,maxnumsteps); Y=NaN(maxnumsteps,6);
...
global tauH tauM Minfinity Hinfinity V_insert
...
Vp=1/Cm*(Gna*m^3*h*(Ena-V)+Gk*n^4*(Ek-V)+Gl*(El-V)+Gca*m^2*h*(Eca-V)+Iinj);
